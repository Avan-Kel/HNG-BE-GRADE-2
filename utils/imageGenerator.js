...
async function generateSummaryImage(countries) {
  ...
  top5.forEach((c, i) => {
    const gdp = Number(c.estimated_gdp) || 0; // safe fallback
    ctx.fillText(`${i + 1}. ${c.name} - ${gdp.toFixed(2)}`, 50, 140 + i * 40);
  });
  ...
  fs.writeFileSync(path.resolve('cache/summary.png'), canvas.toBuffer('image/png'));
}
